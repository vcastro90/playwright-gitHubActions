...
export class Login implements Performable {
    ...
    async performAs(page: any): Promise<void> {
        await Enter.text(this.username).into(LoginUi.LOGIN_PAGE.USERNAME_FIELD).performAs(page);
        await Enter.text(this.password).into(LoginUi.LOGIN_PAGE.PASSWORD_FIELD).performAs(page);
        await Click.on(LoginUi.LOGIN_PAGE.LOGIN_BUTTON).performAs(page);
    }
}